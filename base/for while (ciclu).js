...
var test = {'Коля':200, 'Вася':300, 'Петя':400};
for (vlad in test) //vlad - key, text - obj
{
	alert(vlad); //выведет 'Коля', 'Вася', 'Петя'
}

console.log(Object.values(test)); //'Коля', 'Вася', 'Петя'
--------------------
...
for (vlad in test)
{
	alert(test[vlad]); //выведет 200, 300, 400
}
------------------------------- break -------------------------------
...
for (var i = 0; i < 10; i++) {
  ...
  if (i % 2 == 0) continue; //codul continua daca nu se indeplineste conditia "if"
